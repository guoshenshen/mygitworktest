        ...
        <div class="banner">
            ...
            <div class="banner-right">
                <div class="right_MoreNews">
                    ...
                    <ul class="right-ul" style="margin-top:18px;padding-top:5px;border-top: 1px dashed #ddd;">
                        <c:if test="${mtMixTrainnews != null}">
                            <c:forEach var="news" items="${newestNewsList}" varStatus="status">
                                <li>
                                    <i class="fa fa-caret-right" aria-hidden="true" style="color: #27649c"></i>&nbsp;&nbsp;
                                    <a href="<%=request.getContextPath()%>/mtMixTrainNews/getNewsDetailFromMainPage.do?newsId=${news.newsId }&newsType=0" target="_blank">
                                        ${news.newsTitle }
                                    </a>
                                </li>
                            </c:forEach>
                        ...
                        <c:if test="${msgMessageInfo != null}">
                            <c:forEach var="message" items="${newestMessageList}" varStatus="status">
                                <li>
                                    <i class="fa fa-caret-right" aria-hidden="true" style="color: #27649c"></i>&nbsp;&nbsp;
                                    <a href="<%=request.getContextPath()%>/msgMessageInfo/getNoticeDetailFromMainPage.do?msgId=${message.id }" target="_blank">
                                            ${message.title }
                                    </a>
                                </li>
                            </c:forEach>
                        ...
                        <ul class="carousel-inner">
                            <c:if test="${courseList != null}">
                                <c:forEach var="course" items="${courseList}" varStatus="status">
                                    <li class="carousel-item" style="position:relative">
                                        <a href="<%=request.getContextPath()%>/onlineCourse/detail.do?bookId=${course.course_id}" target="_blank">
                                            <img src="${course.pictureURL}" onerror="imgError({type:0,target:this})" style="height: 190px"/>
                                            <div class="" style="margin:20px auto;width: 150px">
                                                <div class="courseTitle1" style="font-size: 15px;color: #000;margin-top:-10px" title="${course.courseName}">${course.courseName}</div>
                                            </div>
                                        </a>
                                    </li>
                                </c:forEach>
...
<script type="text/javascript">
    var loginData = {
        nextActionUrl: "../mtTrainNews/getNewsDetailFromMainPage.do?newsId=${mtMixTrainnews.newsId}"
    }
    function getCrumbInfo() {
        var message = '<%=request.getAttribute("msgMessageInfo")%>';
...
<script type="text/javascript">
    $(".banner-left img").attr("onerror", "src=/image/defaultPic/news1.png");
</script>
<script type="text/javascript">
    $("#carousel_1").FtCarousel();
    $(function () {
        $("body .message img").addClass("forViewer").on("error", function () {
            imgError({"type": 2, "target": this});
        });

        $("body .message").on("click", "img", function () {
            $(this).imgViewer();
        })

        $("body .carousel-item img").on("error", function () {
            imgError({"type": 0, "target": this});
        });
    });
</script>
</body>
