;(function($){
    ...
    var Carousel = function(con){
        ...
        this.conItemFirst = $(this.conItems[0]);
        this.conItemLast = $(this.conItems[this.conItems.length-1]);
        this.conItemCurrentIndex=0;
        this.flag = true;
        //默认配置参数
        this.settings = {
            width:900,             //幻灯片的宽度
        ...
        this.getCurrentItem=function(){
        	return $(this.conItems[this.conItemCurrentIndex]);
        }
        this.next.click(function(){
            if(self.flag){
                self.flag = false;
                self.rotate("left");
                self.conItemCurrentIndex=(self.conItemCurrentIndex+1)%self.conItems.length;
                if(self.nextFunc){
                	self.nextFunc(self);
                }
            }
        ...
        this.prev.click(function(){
            if(self.flag){
                self.flag = false;
                self.conItemCurrentIndex=(self.conItemCurrentIndex+self.conItems.length-1)%self.conItems.length;
                self.rotate("right");
                if(self.prevFunc){
                	self.prevFunc(self);
                }
    ...
    Carousel.prototype = {
        //设置上一张和下一张大小
        setDirect:function(){
            var prev = $("<div></div>").addClass("content-btn prev-btn").append($("<div></div>").addClass("btn-img"));
            this.con.prepend(prev);
            var next = $("<div></div>").addClass("content-btn next-btn").append($("<div></div>").addClass("btn-img"));
        ...
        setPostOther:function(){
            ...
            var lw = $(rightItem[rightItem.length-1]).width(),
                lh = $(rightItem[rightItem.length-1]).height();
