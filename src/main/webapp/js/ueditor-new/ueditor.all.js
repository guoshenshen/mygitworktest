...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            var ip;
            $.ajax({
	    		url:'tenantAction.do?method=findTenantIP',
	    	    type:"POST",
	    	    async:false,
	    	    dataType:"text",
            ...
            if (opt.length == 1) {
            	if(ci.src.indexOf("http")>-1)
            		ip = "";
                str = '<img src="' + ip+ci.src + '" ' + (ci._src ? ' _src="' + ip+ci._src + '" ' : '') +
                ...
                for (var i = 0; ci = opt[i++];) {
                	if(ci.src.indexOf("http")>-1)
                		ip = "";
                    str = '<p ' + (ci['floatStyle'] == 'center' ? 'style="text-align: center" ' : '') + '><img src="' + ip+ci.src + '" ' +
                        (ci.width ? 'width="' + ci.width + '" ' : '') + (ci._src ? ' _src="' +ip+ ci._src + '" ' : '') +
                        (ci.height ? ' height="' + ci.height + '" ' : '') +
